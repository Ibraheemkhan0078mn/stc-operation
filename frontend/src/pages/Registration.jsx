...
const Registration = () => {
  ...
  const registerUser = async () => {
    try {
      const response = await axios.post(import.meta.env.VITE_backend_base_url + "/auth/registration", {
        name,
        email,
        password,
        role,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        ...
        <p className="text-gray-500 text-center mb-8">Register your account</p>

        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Name</label>
            <input type="text" placeholder="Enter your name" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          ...
          <button onClick={registerUser} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition">
            Register
          </button>
