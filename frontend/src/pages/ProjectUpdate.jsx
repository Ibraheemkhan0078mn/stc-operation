...
const ProjectUpdate = () => {
  ...
  useEffect(()=>{
    async function getProjectData(){
         const response = await axios.post(import.meta.env.VITE_backend_base_url + "/projects/getOneProject", {id: params.id })
         if(response.data.success==true){
            let projectData= response.data.project
            setName(projectData.name)
            setCustomerName(projectData.customerName)
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(import.meta.env.VITE_backend_base_url + "/projects/updateProject", {
        name,
        customerName,
        notes,
        id: params.id
